...
class ApiTokenCookieFactory
{
     ...
     * @return ParagonIE\Halite\Cookie
     */
    public function make($userId, $csrfToken)
    {
        $config = $this->config->get('session');
        $sodiumConfig = $this->config->get('sodium');
        $key = KeyFactory::loadEncryptionKey($sodiumConfig['key_path']['e_path']);
        $cookieStorage = new Cookie($key);

        $expiration = Carbon::now()->addMinutes($sodiumConfig['ttl']);
        return $cookieStorage->store(
            SodiumAuth::cookie(),
